...
function TimetableTitle({ labelColor, className }: TimetableTitleProps) {
	...
	return (
		<div
			...
			className={cn(!isEmpty && "pb-3", className)}
		>
			<input
				type="text"
				value={title}
				...
				className="bg-transparent border-none outline-none ring-0 shadow-none font-bold text-xl p-0 w-full min-w-0 cursor-text placeholder:text-muted-foreground/25"
				style={{
...
function TimetableWatermark({ labelColor }: TimetableWatermarkProps) {
	return (
		...
		>
			...
			<Logo height={12} style={{ fill: labelColor }} aria-label="weekview" />
		</div>
	);
}
