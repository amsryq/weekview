...
// Controlled version: accepts open/onOpenChange, no trigger
export function UiTMImporterDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const { currentStep, setOpen, setCurrentStep, reset } = useImporterSelectionStore(
    useShallow((state) => pick(state, [
      "currentStep",
      "setOpen",
      "setCurrentStep",
      "reset",
...
export function UiTMImporterDialogRenderer() {
  ...
  return <UiTMImporterDialog open={uiTMImporterOpen} onOpenChange={closeUiTMImporter} />;
}
  ...
  return (
    <CourseEditorDialog
      ...
      onSubmit={(data, form) => {
        ...
        const conflicts = CourseStore.getState().getConflictingCourses(course.meetingTimes);
